...
2. Warms up the AgentRegistry
    """
    logger.info("Starting full_stack_agent API")
    ...
    logger.info("Shutting down full_stack_agent API")

app = FastAPI(title="Full Stack Agent API", lifespan=lifespan)
...
def read_root():
    ...
    return {"message": "Welcome to the Full Stack Agent API"}
...
@app.post("/analyse", response_model=QueryResponse)
def handle_query(request: QueryRequest, _key: str = Depends(auth_dep)):
    """ Main endpoint to handle natural language queries """
    query = request.query
    request_id = RequestContext.get_request_id()
