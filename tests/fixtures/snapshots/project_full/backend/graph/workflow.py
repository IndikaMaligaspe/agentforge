...
# When checkpointing is enabled, your ASGI lifespan MUST await init_checkpointer()
# before any request invokes the graph.
from .checkpointer import init_checkpointer, aclose_checkpointer, get_checkpointer
...
def _build_graph() -> StateGraph:
    ...
    - query_router: Classifies queries as 'data', 'analytics'    - supervisor: Dispatches to the appropriate agent based on intent
...
# The base graph above is compiled without persistence. get_compiled_graph()
# attaches the saver after init_checkpointer() has run in the ASGI lifespan.
_CHECKPOINTED_GRAPH = None


def get_compiled_graph():
    """Return the compiled graph with the async saver attached.

    On first call, compiles the graph with the already-initialized
    AsyncPostgresSaver. Subsequent calls return the cached instance.
    Raises RuntimeError if init_checkpointer() has not been awaited first.
    """
    global _CHECKPOINTED_GRAPH
    if _CHECKPOINTED_GRAPH is None:
        _CHECKPOINTED_GRAPH = _build_graph().compile(checkpointer=get_checkpointer())
