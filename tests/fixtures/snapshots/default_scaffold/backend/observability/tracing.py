...
@observe(name="full_featured_project Query")
def trace_agent_run(query: str):
    ...
    tenant_id = RequestContext.get_tenant_id() or "unknown"
    trace_id = RequestContext.get_trace_id() or "unknown"
    ...
    try:
        ...
        if tenant_id:
            trace_tags["tenant_id"] = tenant_id
        if trace_id:
            trace_tags["trace_id"] = trace_id
        ...
        with propagate_attributes(
            ...
            for key, value in RequestContext.get_all_context().items():
                if key not in ["request_id", "user_id", "session_id", "tenant_id", "trace_id"]:
