...
queries as 'sql', 'analytics' and routes them accordingly.
"""
...
def query_router_node(state: AgentState) -> AgentState:
    """
    Classify the user query as 'sql', 'analytics'.
    ...
    try:
        ...
        ROUTER_PROMPT = """Classify the user's question as one of: sql, analytics.
...
Question: {query}"""
        ...
        if intent not in valid_intents:
            log_with_props(logger, "warning", f"Invalid classification response: {intent}. Defaulting to 'sql'",
                          node=node_name,
                          request_id=request_id,
                          raw_response=response.content)
            intent = "sql"  # Default on any parse error
        ...
        # Default to 'sql' on any error
        return {
            **state,
            'intent': 'sql'  # Default on any error
        }