...
def ask_ci_config() -> dict:
    """Interactive prompt for CI configuration."""
    ...
    if provider == "none":
        return {"provider": "none", "python_version": "3.12", "installer": "uv"}

    python_version = questionary.select(
        "Python version for CI:",
        choices=["3.11", "3.12", "3.13"],
        default="3.12",
    ).ask()

    installer = questionary.select(
        "Package installer:",
